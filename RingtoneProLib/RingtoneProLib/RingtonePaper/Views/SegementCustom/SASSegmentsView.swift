...
public class SASSegmentsView: UIView {
    ...
    public var fontsText: UIFont = UIFont.systemFont(ofSize: 14.0){
        didSet{
            segment.setTitleTextAttributes([
                NSAttributedString.Key.font : fontsText,
                NSAttributedString.Key.foregroundColor: colorUnSelectedSegment
                ], for: .normal)
            segment.setTitleTextAttributes([
                NSAttributedString.Key.font : fontsText,
                NSAttributedString.Key.foregroundColor: colorSelectedSegment
                ], for: .selected)
            //self.layoutIfNeeded()
            updateUI()
        }
    ...
    @IBAction private func actionChangeSegment(_ sender: Any) {
        ...
        for index in 0..<self.segment.selectedSegmentIndex {
            ...
            sizeAll = sizeAll + string!.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: fontsText]).width + space
        }
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.layoutWidhtUnderlineView.constant =  string!.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: self.fontsText]).width + self.space
            
            self.layoutIfNeeded()
        }
        
        let thePosition = CGRect.init(x: sizeAll, y: 0, width: string!.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: fontsText]).width + space, height: 5.0)
    ...
    private func updateUI(){
        ...
        if self.segment.numberOfSegments > 0{
            ...
            for index in 0..<self.segment.numberOfSegments{
                let string = self.segment.titleForSegment(at: index)
                sizeAll = sizeAll + string!.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: fontsText]).width
            }
            ...
            layoutWidhtUnderlineView.constant =  string!.size(withAttributes: [kCTFontAttributeName as NSAttributedString.Key: fontsText]).width + space

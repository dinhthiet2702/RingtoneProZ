...
//			_currentContentController.view.frame = _containerController.view.bounds;
	...
	void (^animationBlock)(void) = ^
	{
		...
		{
			[self->_currentContentController _ln_beginAppearanceTransition:NO animated:YES];
		}
	...
	void (^completionBlock)(BOOL) = ^(BOOL finished)
	{
		...
		if(state == LNPopupPresentationStateBarPresented)
		{
			[self->_currentContentController _ln_endAppearanceTransition];
			
			[self _cleanupGestureRecognizersForController:self->_currentContentController];
			
			[self->_currentContentController.viewForPopupInteractionGestureRecognizer removeGestureRecognizer:self.popupContentView.popupInteractionGestureRecognizer];
			...
			if(self->_popupControllerPublicState == LNPopupPresentationStateOpen)
			{
				_LNCallDelegateObjectBool(self->_containerController, @selector(popupPresentationControllerDidClosePopup:animated:), animated);
			}
		}
		else if(state == LNPopupPresentationStateOpen)
		{
			...
			[self->_currentContentController.viewForPopupInteractionGestureRecognizer addGestureRecognizer:self.popupContentView.popupInteractionGestureRecognizer];
			[self _fixupGestureRecognizersForController:self->_currentContentController];
			...
			if(self->_popupControllerPublicState == LNPopupPresentationStateBarPresented)
			{
				_LNCallDelegateObjectBool(self->_containerController, @selector(popupPresentationControllerDidOpenPopup:animated:), animated);
			}
		}
		
        self->_popupControllerInternalState = state;
		if(state != _LNPopupPresentationStateTransitioning)
		{
			[self->_containerController _ln_setPopupPresentationState:state];
		}
	...
	if(resolvedStyle == LNPopupInteractionStyleSnap)
	{
		if((_popupControllerInternalState == LNPopupPresentationStateBarPresented && [pgr velocityInView:self.popupBar].y < 0))
		{
			...
			dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
				[self _transitionToState:self->_popupControllerTargetState notifyDelegate:YES animated:YES useSpringAnimation:self->_popupControllerTargetState == LNPopupPresentationStateBarPresented ? YES : NO allowPopupBarAlphaModification:YES completion:nil transitionOriginatedByUser:NO];
			});
		}
...
- (void)_configurePopupBarFromBottomBar
{
	...
	dispatch_async(dispatch_get_main_queue(), ^{
		self.popupBar.effectGroupingIdentifier = self->_bottomBar._effectGroupingIdentifierIfAvailable;
	});
